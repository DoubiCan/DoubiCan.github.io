...
paperlink1: 'https://DoubiCan.github.io/files/AModel%20fortheDetectionofMovingTargets%20inVisual_wiederman2008.pdf'
paperlink2: 'https://DoubiCan.github.io/files/An%20autonomous%20robot%20inspired%20by%20insect%20neurophysiology%20pursues%20moving%20features%20in%20natural%20environments2017.pdf'
tags:
  - Paper Handout
  - Code
  - Filter
...
<a href="https://DoubiCan.github.io/files/AModel%20fortheDetectionofMovingTargets%20inVisual_wiederman2008.pdf">paper1</a>
<a href="https://DoubiCan.github.io/files/An%20autonomous%20robot%20inspired%20by%20insect%20neurophysiology%20pursues%20moving%20features%20in%20natural%20environments2017.pdf">paper2</a>
...
<div style="text-align: center;">
  ...
  <img src='/images/STMD2008/2008STMD资料补充3.png'>
  <p>补充资料</p>
</div>
...
<div style="text-align: center;">
  <img src='/images/STMD2008/FDSR.png'>
  <p>FDSR</p>
</div>
...
<div style="text-align: center;">
  <img src='/images/STMD2008/Fail_2008Repetition.png'>
  <p>复刻失败</p>
</div>
...
class STMD():
    def __init__(self,video_size,conv_params):
        ...
        self.Inhibition_output=torch.zeros(video_size, dtype=torch.float64)
        ...
        self.tempoutput13=torch.zeros(video_size, dtype=torch.float64)
        self.tempoutput14=torch.zeros(video_size, dtype=torch.float64)
        ...
        self.tempoutput9=torch.zeros(video_size, dtype=torch.float64)
        self.count=0
        self.k=0
    ...
    def Lamina(self):
        ...
        self.tempoutput13=self.Gamma_Conv3d_forward(self.RetinaOutput_cache[index,:,:],self.GammaConv3d_1) if self.count==15 else self.tempoutput13
        self.tempoutput14=self.Gamma_Conv3d_forward(self.RetinaOutput_cache[index,:,:],self.GammaConv3d_2) if self.count==15 else self.tempoutput14
...
# axises=[(61,168),(126,168),(197,168),(280,168),(387,168)]
axises=[(62,168),(126,168),(197,168),(280,168),(387,168)]
plot_num=len(axises)
plot_I,plot_LMC,plot_Lamina,plot_ON,plot_OFF,plot_OFFDELAY,plot_Inhibition,plot_Lobula=[],[],[],[],[],[],[],[]
# 定义要收集的层信息和对应的模型输出路径
layers_info = [
    ('Retina', 'RetinaOutput_cache', -1),
    ('Lamina', 'LaminaOutput', -1),
    ('ON', 'MedullaOutput1_ON', -1),
    ('OFF', 'OFF_cache', -1),
    ('OFFDELAY', 'MedullaOutput2_OFFDELAY', -1),
    ('Inhibition','Inhibition_output',-1),
    ('Lobula', 'LobulaOutputoutput', -1)
]
# 初始化数据结构：每个坐标点对应一个列表
plot_data = {
    name: {coord: [] for coord in axises}  # 为每个坐标点创建独立列表
    for name, _, _ in layers_info
}
...
for i in tqdm(range(T), desc="Processing frames"):#frame=0:T-1
    ...
    ######################store for draw#######################
    frame_data = [
        model.RetinaOutput_cache[-1,:,:].clone(),        # 视频帧（H,W,3）
        model.LaminaOutput.clone(),                     # 运动检测（H,W）
        model.MedullaOutput1_ON.clone(),
        model.OFF_cache[-1,:,:].clone(),
        model.MedullaOutput2_OFFDELAY.clone(),      # Retina输出（H,W）
        model.LobulaOutputoutput.clone()
    ...
    for coord in axises:
        # 收集每个层在当前位置的数据
        for name, attr, _ in layers_info:
            
            layer_output = getattr(model, attr)
            layer_output=layer_output[-1,:,:].clone() if layer_output.ndim==3 else layer_output.clone()
            
            value = layer_output[coord[0], coord[1]]  # 注意坐标顺序是 (H, W) 还是 (W, H)
            plot_data[name][coord].append(value)


```

<!-- <div style="text-align: center;">
  <img src='/images/STMD2008/test.gif' alt='动态示例' style='width: 1000px; height: auto;'>
  <p>GIF 动态演示效果</p>
</div> -->

<div style="text-align: center;">
  <img src='/images/STMD2008/f(xi,yi,t).png' style="width: 1280px; height: auto; max-width: 100%;">
  <p>f(xi,yi,t)</p>
</div>
...
<source src="https://doubican.github.io/images/STMD2008/cell_matries.mp4" type="video/mp4">
您的浏览器不支持 video 标签。</video>

<video width="1280" height="720" controls autoplay loop>
<source src="https://doubican.github.io/images/STMD2008/Translating_5_boxes.mp4" type="video/mp4">
您的浏览器不支持 video 标签。</video>


## Remaining Questions
1. 老师给到的提醒1:按理来说，ori-FDSR(ori)应该=0而不会有脉冲余留，请进行代码的数值检查
2. 老师给到的提醒1:第二篇文章关于不同起伏平面的频率图的解释原文是怎么说的


---
<!-- 
<a href="https://github.com/DoubiCan/DoubiCan.github.io/raw/refs/heads/master/images/STMD2008/Translating_5_boxes.mp4">downloadlink</a>

<a href="https://doubican.github.io/images/STMD2008/Translating_5_boxes.html">test2.mp4</a>

maybe helpful:
https://medium.com/@ciaranbench/how-to-add-a-video-file-on-github-pages-github-io-website-html-a2e9dd81618a

https://github.com/orgs/community/discussions/48223 -->
