...
<div style="text-align: center; margin: 0 auto;">
  ...
  <div style="display: inline-block; width: 32%; vertical-align: top;">
    ...
    <p style="margin: 10px 0; font-weight: bold; font-size: 14px;">frame_viz_113</p>
  </div>
  <div style="display: inline-block; width: 32%; vertical-align: top;">
    <img src='/images/Event_STMD_MathV1/Experiment real environment/events/Response_frame_113_ESTMD.jpg' alt="Response_frame_113_ESTMD" style="max-width: 95%;  display: block; margin: 0 auto;">
    <p style="margin: 10px 0; font-weight: bold; font-size: 14px;">Response_frame_113_ESTMD</p>
  </div>
  <div style="display: inline-block; width: 32%; vertical-align: top;">
    <img src='/images/Event_STMD_MathV1/Experiment real environment/events/Response_frame_113_Feedback_stmd.jpg' alt="Response_frame_113_Feedback_stmd" style="max-width: 95%; display: block; margin: 0 auto;">
    <p style="margin: 10px 0; font-weight: bold; font-size: 14px;">Response_frame_113_Feedback_stmd</p>
  </div>
  <!-- 图注 -->
...
<div style="text-align: center; margin: 0 auto;">
  ...
  <div style="display: inline-block; width: 49%; vertical-align: top;">
    ...
    <p style="margin: 10px 0; font-weight: bold; font-size: 14px;">frame_113_video</p>
  </div>
  <div style="display: inline-block; width: 49%; vertical-align: top;">
    <img src='/images/Event_STMD_MathV1/Experiment real environment/video_para2/Response_frame_LMC_116_video_para2.jpg' alt="Response_frame_LMC_116_video_para2" style="max-width: 95%;  display: block; margin: 0 auto;">
    <p style="margin: 10px 0; font-weight: bold; font-size: 14px;">Response_frame_LMC_116_video_para2</p>
  </div>
  <!-- 第二行 -->
  <div style="display: inline-block; width: 49%; vertical-align: top;">
    <img src='/images/Event_STMD_MathV1/Experiment real environment/video_para2/Response_frame_ESTMD_116_video_para2.jpg' alt="Response_frame_ESTMD_116_video_para2" style="max-width: 95%; display: block; margin: 0 auto;">
    <p style="margin: 10px 0; font-weight: bold; font-size: 14px;">Response_frame_ESTMD_116_video_para2</p>
  </div>
  <div style="display: inline-block; width: 49%; vertical-align: top;">
    <img src='/images/Event_STMD_MathV1/Experiment real environment/video_para2/Response_frame_Feedback_113_video_para2.jpg' alt="Response_frame_Feedback_113_video_para2" style="max-width: 95%; display: block; margin: 0 auto;">
    <p style="margin: 10px 0; font-weight: bold; font-size: 14px;">Response_frame_Feedback_113_video_para2</p>
  </div>
  <!-- 图注 -->
  <div style="clear: both; padding-top: 20px;">
...
## Gamma_Conv_Realization
还有一点之前没有发现的问题是,本以为数学上完全等价的Gamma滤波实现，在实际上并不一样，见下面测试。区别主要是在微分方程这种实现方式在极值点和滤波强度上不同。

<div style="text-align: center;">
  <img src='/images/Event_STMD_MathV1/gamma_conv.png'>
  <p>不同gamma滤波实现的区别</p>
</div>


<!-- <div style="overflow-x: auto; overflow-y: hidden; background-color: #f5f5f5; padding: 10px; border-radius: 4px; border: 1px solid #e1e4e8;">
...
def gamma_kernel_conv(current_input, Xpast, tau, order, 
                     method, **kwargs):
    if method == 'Recursive':
        # 微分方程实现 ========================
        ...
        Xnew = np.zeros_like(Xpast)
        # 第0阶：当前输入
        Y1 = Xpast[..., 1:(order+1)]  # 上一时刻的1~order阶
        Y2 = Xpast[..., 0:order]       # 上一时刻的0~order-1阶
        Y3 = (1 - Mu) * Y1 + Mu * Y2

        Xnew[..., 1:(order+1)] = Y3   
        Xnew[..., 0] = current_input
        
        return Xnew  # shape=[H, W, order+1]
    
    else:
        # 卷积核实现 ========================
        # 从kwargs获取必需参数
        gamma_kernel = kwargs['gamma_kernel']  # 预计算的卷积核 [residual_len]
        residual_len = kwargs['residual_len']  # 缓冲区长度
        
        # 初始化缓冲区（首次调用）
        if Xpast is None or Xpast.shape[-1] != residual_len:
            buffer = np.zeros((*current_input.shape, residual_len), dtype=np.float64)dawwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww
        else:
            buffer = Xpast.copy()
        
        # 更新缓冲区：移除最早的数据，添加当前输入
        buffer = np.roll(buffer, shift=-1, axis=-1)  # 向右滚动
        buffer[..., -1] = current_input  # 确保索引正确
        
        # 执行卷积运算（在时间轴上）
        # axes=(-1, 0)表示在buffer的-1维度和gamma_kernel的0维度上进行卷积
        conv_result = np.tensordot(buffer, gamma_kernel, axes=(-1, 0))
        
        return conv_result, buffer  # 返回结果和更新后的缓冲区
</code></pre>
</div>