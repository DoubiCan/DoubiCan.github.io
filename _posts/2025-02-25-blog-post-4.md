...
这里的\\(N_{on}\\)将被保守地考虑为\\(\triangleq N_{on\_rea}}-N_{off\_real}=\tiny{时间窗口内发生的on事件数-时间窗口内发生的off事件数}\\),(这种不考虑\\(N_{on}\triangleq N_{on\_ real}\\)的保守性实际上是为了接下来的行波性质分析)。
...
3. 关于\\(N_{onreal}-N_{offreal}\\)是可以取到负数的，这与原本stmd的正部非负性是不同的，所以我似乎遗漏了一个max(\\(N_{onreal}-N_{offreal}\\),0)的取正部操作,得修改一下了...