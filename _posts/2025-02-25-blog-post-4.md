...
<pre><code class="python">def gamma_kernel_conv(current_input, Xpast, tau, order, 
                     method, **kwargs):
    if method == 'Recursive':
        # 微分方程实现 ========================
        ...
        if Xpast is None or Xpast.shape[-1] != residual_len:
            buffer = np.zeros((*current_input.shape, residual_len), dtype=np.float64)
        else:
            buffer = Xpast.copy()
        ...
        return conv_result, buffer  # 返回结果和更新后的缓冲区</code></pre>
</div>
